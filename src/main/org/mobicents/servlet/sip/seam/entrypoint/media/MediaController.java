...
public class MediaController {
	 ...
	 * @param events
	 */
	public void execute(MsEndpoint endpoint, MsRequestedSignal[] signals,
			MsRequestedEvent[] events) {
		endpoint.execute(signals, events);
		if (MediaControllerManager.instance().listenerMap.get(endpoint) == null) {
			MsNotificationListener newListener = new NotificationListener(
					sipSession, msSession, endpoint, null);
			MediaControllerManager.instance().listenerMap.put(endpoint,
					newListener);
	 ...
	 */
	public void execute(MsEndpoint endpoint, MsRequestedSignal[] signals,
			MsRequestedEvent[] events, MsConnection connection) {
		endpoint.execute(signals, events, connection);
		if (MediaControllerManager.instance().listenerMap.get(connection) == null) {
			MsNotificationListener newListener = new NotificationListener(
					sipSession, msSession, endpoint, connection);
			MediaControllerManager.instance().listenerMap.put(connection,
					newListener);
	 ...
	 */
	public void execute(MsEndpoint endpoint, MsRequestedSignal[] signals,
			MsRequestedEvent[] events, MsLink link) {
		endpoint.execute(signals, events, link);
		if (MediaControllerManager.instance().listenerMap.get(link) == null) {
			MsNotificationListener newListener = new NotificationListener(
					sipSession, msSession, endpoint, link);
			MediaControllerManager.instance().listenerMap
					.put(link, newListener);
