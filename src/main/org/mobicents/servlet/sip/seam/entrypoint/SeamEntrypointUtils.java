...
public class SeamEntrypointUtils {
	private static LogProvider log = Logging
			.getLogProvider(SeamEntrypointUtils.class);
	...
	public static void destroyContext(Context context) {
		try {
			Method destroyMethod = Contexts.class.getDeclaredMethod("destroy",
					Context.class);
	...
	public static void beginEvent(SipServletMessage message) {
		...
		setContext("applicationContext", new ApplicationContext(Lifecycle
				.getApplication()));
		setContext("eventContext", eventContext);
		setContext("sessionContext", new SessionContext(
				new SipSeamRequestSessionMap(message)));
		setContext("conversationContext", new BasicContext(
	...
	public static void beginEvent(SipSession sipSession) {
		...
		setContext("sessionContext", new SessionContext(new SipSeamSessionMap(
				sipSession)));
		setContext("conversationContext", new BasicContext(
				ScopeType.CONVERSATION));
		setContext("businessProcessContext", new BusinessProcessContext());
	...
	private static void simulateConversationStart(SipSession session) {
		...
		Manager.instance().setCurrentConversationIdStack(
