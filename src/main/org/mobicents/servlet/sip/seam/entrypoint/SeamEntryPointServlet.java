...
public class SeamEntryPointServlet extends javax.servlet.sip.SipServlet
		implements
			SipSessionListener {
	private static LogProvider log = Logging
			.getLogProvider(SeamEntryPointServlet.class);
	...
	public void sessionCreated(final SipSessionEvent arg0) {
		Thread t = new Thread() {
			...
			public void run() {
				...
				Lifecycle
						.beginSession(new SipSeamSessionMap(arg0.getSession()));
				...
				Events.instance().raiseEvent("sipSessionCreated",
						arg0.getSession());
	...
	protected void doRequest(SipServletRequest request)
		...
		if (init != null && init.isDebug()) {
			...
			try {
				...
				log
						.error(
								"Unexpected exception while movking the servlet context",
								e);
			}
		}
		String dtmf = checkForSipDtmf(request);
		SeamEntrypointUtils.beginEvent(request);
		Contexts.getApplicationContext().set("sipFactory",
				(SipFactory) getServletContext().getAttribute(SIP_FACTORY));
		Contexts.getApplicationContext().set(
				"sipSessionsUtil",
				(SipSessionsUtil) getServletContext().getAttribute(
						SIP_SESSIONS_UTIL));
		Contexts.getApplicationContext().set("timerService",
				(TimerService) getServletContext().getAttribute(TIMER_SERVICE));
		Events.instance()
				.raiseEvent(request.getMethod().toUpperCase(), request);
		if (dtmf != null) {
			...
			if (request.getMethod().equalsIgnoreCase("INFO")) {
				...
				if (contentType != null) {
					if ("application/dtmf".equalsIgnoreCase(contentType.trim())) {
						String messageContent = new String((byte[]) request
								.getContent());
						log.debug("Detected application/dtmf");
						return messageContent.trim();
					} else if ("application/dtmf-relay"
						...
						if (messageContent != null
								...
								&& signalIndex != -1) {
							String signal = messageContent.substring(
